...
let lightfragurl = "./shaders/light.frag"
let lightverturl = "./shaders/light.vert"
let lightposurl = "./shaders/lightposition.frag"
let lightvelourl = "./shaders/lightvelo.frag"

let lightfrag, lightvert, lightpos, lightvelo

Promise.all([
    new THREE.FileLoader().loadAsync(lightfragurl),
    new THREE.FileLoader().loadAsync(lightverturl),
    new THREE.FileLoader().loadAsync(lightvelourl),
    new THREE.FileLoader().loadAsync(lightposurl)
]).then(shader => {
    lightfrag = shader[0]
    lightvert = shader[1]
    lightvelo = shader[2]
...
class BirdGeometry extends THREE.BufferGeometry {
  constructor() {
    ...
    const vertices = new THREE.BufferAttribute(new Float32Array(points), 3);
    ...
    for (let f = 0; f < BIRDS; f++) {
      ...
      verts_push(0, -0, -20, 0, 40, -10, 0, 0, 10);

      // Wings
    }

    for (let v = 0; v < triangles ; v++) {
      const birdIndex = ~~(v / trianglesPerBird);
      const x = birdIndex % WIDTH / WIDTH;
      const y = ~~(birdIndex / WIDTH) / WIDTH;

      const c = new THREE.Color(0xFFFFFF);

      birdColors.array[v + 0] = c.r;
      birdColors.array[v + 1] = c.g;
      birdColors.array[v + 2] = c.b;
...
var L1 = new THREE.PointLight(0xB887ED, 1.5);
  ...
  const cube = new THREE.Mesh( boxgeometry, greyPhongMat );
  scene.add( cube );
  initBirds();
  test();
}
function test() {
  ...
  // }
  console.log(gpuCompute.getCurrentRenderTarget(
    positionVariable
  ).texture)
}
